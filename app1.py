"""
Stroke Mortality Risk Prediction - Clinical Decision Support System
...
# Page configuration
st.set_page_config(
    page_title="Stroke Mortality Risk Prediction",
    page_icon="🏥",
    layout="wide",
...
def get_openai_client():
    ...
    if not api_key:
        st.warning("⚠️ OpenAI API key not found. Please set OPENAI_API_KEY environment variable or add to Streamlit secrets.")
        return None
...
def load_models():
    ...
    try:
        ...
        st.error("❌ Model files not found. Please ensure 'stroke_mortality_model_bundle.pkl' is in the same directory.")
        st.stop()
...
def create_feature_dict(form_data, all_features):
    ...
    # Blood pressure
    features['blood_pressure'] = 1 if form_data['blood_pressure'] == 'Abnormal' else 0
    ...
    for ms in marital_statuses:
        features[f'care_level_{ms}'] = 1 if form_data['marital_status'] == ms else 0
    ...
    entity_features = {
        ...
        'pct_BIOLOGICAL_STRUCTURE': form_data.get('pct_BIOLOGICAL_STRUCTURE', 0.05),
        'pct_LAB_VALUE': form_data.get('pct_LAB_VALUE', 0),
        'pct_DIAGNOSTIC_PROCEDURE': form_data.get('pct_DIAGNOSTIC_PROCEDURE', 0.05),
        'pct_SEVERITY': form_data.get('pct_SEVERITY', 0.10),
        'pct_THERAPEUTIC_PROCEDURE': form_data.get('pct_THERAPEUTIC_PROCEDURE', 0),
        'pct_CLINICAL_EVENT': form_data.get('pct_CLINICAL_EVENT', 0),
        'pct_DETAILED_DESCRIPTION': form_data.get('pct_DETAILED_DESCRIPTION', 0),
...
def get_top_risk_factors(shap_values_dict, feature_names, top_n=5):
    ...
    all_shap = np.array([shap_values_dict[h] for h in sorted(shap_values_dict.keys())])
...
def generate_llm_explanation(patient_data, risk, horizon, top_factors, shap_values, feature_names):
    ...
    top_factors_text = []
    for i, (feat, shap_val) in enumerate(zip(top_factors[:5], shap_values[:5])):
        direction = "increases" if shap_val > 0 else "decreases"
        patient_value = patient_data.get(feat, "N/A")
        top_factors_text.append(f"{i+1}. {feat} = {patient_value} ({direction} risk by {abs(shap_val):.3f})")
...
Use clear, professional language suitable for clinicians. Do not use bullet points."""
    ...
    try:
        response = client.chat.completions.create(
            model="gpt-4",
            messages=[
                {"role": "system", "content": "You are an expert clinical AI assistant specializing in stroke care and mortality risk assessment."},
...
def plot_risk_trajectory(risks):
    ...
    # Plot line
    ax.plot(horizons, risk_values, marker='o', linewidth=3, markersize=12, 
            color='#1f77b4', label='Mortality Risk')
    ...
    ax.set_title('Patient Mortality Risk Trajectory', fontsize=16, fontweight='bold')
    ax.set_ylim([0, 1])
    ...
    for h, r in zip(horizons, risk_values):
        ax.annotate(f'{r:.1%}', xy=(h, r), xytext=(0, 10), 
...
def plot_top_risk_factors(top_features, top_values, patient_features):
    ...
    ax.set_xlabel('Average Impact on Risk (SHAP value)', fontsize=12, fontweight='bold')
    ax.set_title('Top 5 Risk Factors Across All Time Horizons', fontsize=14, fontweight='bold')
...
def main():
    ...
    # Header
    st.markdown('<div class="main-header">🏥 Stroke Mortality Risk Prediction System</div>', 
                unsafe_allow_html=True)
    ...
    with st.sidebar.form("patient_form"):
        ...
        age_group = st.selectbox(
            "Age Group",
            options=[1, 2, 3, 4, 5],
            format_func=lambda x: {1: "18-44", 2: "45-64", 3: "65-74", 4: "75-84", 5: "85+"}.get(x),
            index=2
        )
    ...
    if submit_button:
        ...
        with st.spinner("🔄 Analyzing risk factors..."):
            shap_values_dict = compute_shap_values(patient_features, models, explainers, feature_names)
            top_features, top_values = get_top_risk_factors(shap_values_dict, feature_names, top_n=5)
    ...
    if st.session_state.prediction_made:
        ...
        fig_factors = plot_top_risk_factors(top_features, top_values, patient_features)
        ...
        if st.button("Generate Explanation", type="primary", key="generate_explanation"):
            with st.spinner("🤖 Generating AI explanation..."):
                ...
                explanation = generate_llm_explanation(
                    form_data,
                    risks[selected_horizon],
                    selected_horizon,
                    horizon_top_features,
                    horizon_top_shap,
                    feature_names
                )
        ...
        report = {
            ...
            **{f'{h}_Day_Risk': f"{risks[h]:.3f}" for h in sorted(risks.keys())}
        }
        ...
        st.info("👈 Please fill in the patient information form in the sidebar and click 'Predict Risk' to begin assessment.")
